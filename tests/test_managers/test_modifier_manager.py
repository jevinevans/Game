...
def test_modifier_manager_build_modifier_add_mod(m_list_select, m_str_val, m_num_range, m_yn_val, m_db_update):
...
@patch("funclg.managers.modifier_manager.string_validation")
@patch("funclg.managers.modifier_manager.list_choice_selection")
def test_modifier_manager_build_modifier_mult_mod(m_list_select, m_str_val, m_num_range, m_yn_val, m_db_update):
    # Test new build method (no name, no from method)
    m_str_val.side_effect = ["Test_mod"]
    ...
    mod_man.build_modifier()
    test_mod = mod_man.Modifier("Test_mod", mults={"health":.6})
    m_db_update.assert_called_with(mod_man.MODIFIER_DATA, test_mod.export())
...
def test_modifier_manager_build_modifier_return_mod(m_list_select, m_num_range, m_yn_val):
    ...
    # Tests return value
    m_list_select.side_effect = ["energy", "Base Change", "attack", "Base Change", "health", "Percentage Change","defense" , "Percentage Change"]
    ...
    test_mod = mod_man.Modifier("Test_mod", adds={"energy":50, "attack":250}, mults={"health":.6, "defense":.75})
