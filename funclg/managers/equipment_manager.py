"""
Programmer: Jevin Evans
Date: 6.19.2022
Description: A manager class for creating, updating, and removing equipment.
"""

from loguru import logger

import funclg.managers.stats_manager as stats_man
from funclg.character.equipment import BodyEquipment, WeaponEquipment
from funclg.managers.manager import BaseManager, SingletonMeta
from funclg.utils.types import ARMOR_TYPES, ITEM_TYPES, WEAPON_TYPES


class EquipmentManager(BaseManager, metaclass=SingletonMeta):
    """
    A manager class for creating, updating, and removing equipment.
    """

    def __init__(self):
        """Initialize the EquipmentManager."""
        super().__init__(name="Equipment", filename="equipment.json")
        self.menu["menu_items"] = [
            {"title": "Add New Equipment", "value": self.build_equipment},
            # {"title": "Edit Equipment", "value": self.edit_equipment},
            {"title": "View Equipment Details", "value": self.show_equipment},
            {"title": "Delete Equipment", "value": self.delete_equipment},
        ]
        logger.debug("EquipmentManager initialized.")
        self.load_data()
        logger.debug(f"EquipmentManager data loaded {len(self.data)} equipment.")

    def update_data(self):
        for _id, data in self.data.items():
            if _id not in self.objects:
                logger.debug("adding data")
                if data["item_type"] == 4:
                    self.objects[_id] = WeaponEquipment(**data)
                else:
                    self.objects[_id] = BodyEquipment(**data)
        logger.debug("Function complete")
        super().update_data()

    def export_data(self):
        for _id, data in self.objects.items():
            self.data[_id] = data.export()

        super().export_data()

    def _new_weapon(self):
        print()
        weapon_type = self.get_selection(
            "What kind of Weapon would you like to create?", list(WEAPON_TYPES)[:-1]
        )
        weapon_name = self.get_string(
            f"What would you like to name this new {weapon_type}?", "Name"
        )
        weapon_desc = self.get_string(f"How would you describe {weapon_name}?", "Description")

        print("Generating stats for this weapon...")
        weapon_stat = stats_man.build_stats()

        return WeaponEquipment(
            name=weapon_name,
            weapon_type=weapon_type,
            description=weapon_desc,
            stats=weapon_stat,
        )

    def _new_body_armor(self):
        item_type = self.get_selection(
            "What type of armor would you like to create?", ITEM_TYPES[:-1]
        )
        armor_type = self.get_selection(
            f"What type of armor would you like to make the {item_type}?", ARMOR_TYPES
        )
        item_name = self.get_string(
            f"What would you like to name this new {armor_type} {item_type}?", "Name"
        )
        item_desc = self.get_string(f"How would you describe {item_name}?", "Description")

        print(f"Generating mods for this {item_type}...")
        item_stat = stats_man.build_stats()

        return BodyEquipment(
            name=item_name,
            stats=item_stat,
            description=item_desc,
            armor_type=ARMOR_TYPES.index(armor_type),
            item_type=ITEM_TYPES.index(item_type),
        )

    def build_equipment(self):
        """Dialog for building new equipment"""
        equip_type = self.get_selection(
            "Starting Equipmet Creation:\n\nWhat type of equipment would you like to create:",
            ["Armor", "Weapon"],
        )
        new_equipment = None

        if equip_type == "Armor":
            new_equipment = self._new_body_armor()
        else:
            new_equipment = self._new_weapon()

        if self.get_confirmation(
            f"You created:\n{new_equipment.details()}\nSave new {equip_type}?"
        ):
            self.data[new_equipment.id] = new_equipment.export()
            self.update_data()
            print(f"{new_equipment.name} has been saved!!!")
            return

        print(f"No new {equip_type.lower()}, oh well...")
        del new_equipment

    def filter_equipment_by_armor_type(self, armor_type: int):
        filtered_equipment = {}

        for index, item_type in enumerate(ITEM_TYPES):
            filtered_equipment[item_type] = {
                equipment.id: equipment
                for equipment in self.objects.values()
                if equipment.armor_type == armor_type and equipment.item_type == index
            }

        return filtered_equipment

    def select_equipment(self):
        if self.data:
            equip_list = {}
            choice = self.get_selection(
                "Which type of equipment would you like to select:", ["Weapons", "Armor"]
            )
            if choice == "Armor":
                equip_list = {
                    _id: data for _id, data in self.data.items() if data["item_type"] != 4
                }
            else:
                equip_list = {
                    _id: data for _id, data in self.data.items() if data["item_type"] == 4
                }
            return self.get_selection(f"Please select a {choice}", equip_list, "name", "_id")
        logger.warning("There are is no equipment available.")
        return None

    def show_equipment(self):
        show_equip_id = self.select_equipment()
        if show_equip_id:
            show_equip = self.objects[show_equip_id]
            print(show_equip.details())
            return
        logger.warning("There are no equipment items to show.")

    def delete_equipment(self):
        del_equip_id = self.select_equipment()
        if del_equip_id:
            del_equip = self.data[del_equip_id]
            if self.get_confirmation(f"Do you want to delete \"{del_equip['name']}\"?"):
                print(f"Deleting {del_equip['name']}")
                del self.data[del_equip_id]
                del self.objects[del_equip_id]
                self.update_data()
                return
            print("Keeping all equipment in the vault...")
            return
        logger.warning("There are currently no equipment to delete.")
